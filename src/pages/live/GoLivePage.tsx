...
import { isGuidelinesGateError } from '../../hooks/useStreamGuidelines';
...
export function GoLivePage() {
	...
	const [guidelinesRequireAccept, setGuidelinesRequireAccept] = useState(false);
	...
	function runGoLive() {
		...
		return ensureAuth()
			...
			.catch((e: unknown) => {
				...
				if (isGuidelinesGateError(msg)) {
					pendingGoLiveRef.current = true;
					setGuidelinesRequireAccept(true);
					setShowGuidelinesModal(true);
					setGoLiveError('Accept stream guidelines before going live.');
					return;
				}
				setGoLiveError(msg);
			});
	...
	function handleGuidelinesAccepted() {
		setShowGuidelinesModal(false);
		setGuidelinesRequireAccept(false);
		if (pendingGoLiveRef.current) {
		...
		return (
			<>
				...
				<StreamGuidelinesModal
					...
					requireAccept={guidelinesRequireAccept}
					onAccepted={handleGuidelinesAccepted}
				/>
			</>
		);
	}
