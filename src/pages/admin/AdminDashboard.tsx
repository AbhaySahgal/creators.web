...
export function AdminDashboard() {
	...
	const stats = [
		...
		{ label: 'Total Content', value: mockPosts.length.toString(), icon: TrendingUp, color: 'bg-white/10 text-white/60', path: '/admin/moderation' },
	];
		...
		<div className="min-h-screen bg-[#0d0d0d]">
			...
			<div className="max-w-6xl mx-auto px-4 pt-20 pb-8">
				<div className="mb-6">
					...
					<p className="text-white/40 text-sm">Platform management and moderation</p>
				</div>
				...
				<div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
					{stats.map(({ label, value, icon: Icon, color, path }) => (
						<button
							...
							className="bg-[#161616] border border-white/5 rounded-2xl p-4 text-left hover:border-white/10 transition-all group"
						>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
					<div className="bg-[#161616] border border-white/5 rounded-2xl p-4">
						<div className="flex items-center justify-between mb-3">
							<h3 className="font-semibold text-white text-sm">Top Creators</h3>
							...
							{mockCreators.filter(c => c.isKYCVerified).slice(0, 4).map(creator => (
								<div key={creator.id} className="flex items-center gap-3">
									...
									<div className="flex-1 min-w-0">
										...
										<p className="text-xs text-white/40">{creator.subscriberCount.toLocaleString()} subscribers</p>
									</div>
						...
						<div className="space-y-3">
							{mockReports.map(report => (
								<div key={report.id} className="flex items-start gap-2">
									...
									<div className="flex-1 min-w-0">
										<p className="text-xs font-medium text-white/70 truncate">{report.reason}</p>
										<p className="text-[10px] text-white/30">{report.reporterName} · {report.targetType}</p>
									</div>
				...
				<div className="bg-[#161616] border border-white/5 rounded-2xl p-4">
					<h3 className="font-semibold text-white text-sm mb-3">Platform Revenue Trend</h3>
					<div className="flex items-end gap-1.5 h-20">
						{[28, 35, 40, 44, 42, 48].map((val, i) => (
							<div key={i} className="flex-1 flex flex-col items-center gap-1">
								...
								/>
								<p className="text-[9px] text-white/30">{['Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan'][i]}</p>
							</div>
						))}
					</div>
				</div>
