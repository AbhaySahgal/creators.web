		...
		<div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4">
			...
			<div className="relative w-full sm:max-w-md bg-[#141414] border border-white/10 rounded-t-3xl sm:rounded-3xl animate-slide-up sm:animate-fade-in overflow-hidden">
				...
				<div className="p-5 space-y-5">
					...
					>
						{!selectedType ? 'Select a session type' : !selectedDuration ? 'Select duration' : !canAfford ? 'Insufficient balance' : `Start ${selectedType === 'chat' ? 'Chat' : selectedType === 'audio' ? 'Audio Call' : 'Video Call'} — $${totalCost.toFixed(2)}`}
					</button>
				</div>
			</div>
		</div>
