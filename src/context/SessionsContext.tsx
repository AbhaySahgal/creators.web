...
export function SessionsProvider({ children }: { children: React.ReactNode }) {
	...
	const applyEndedState = useCallback((payload: SessionsEndedEvent) => {
		...
		clearLocalTimer();
	}, [clearLocalTimer, state.active?.accepted, ws]);
	...
	useEffect(() => {
		...
		const onFeedbackPrompt = (data: unknown) => {
			...
			if (roomId && !state.endedRooms[roomId]) {
		...
		const onTimerLike = (data: unknown) => {
			...
			if (state.endedRooms[payload.room_id] || state.ended?.room_id === payload.room_id) return;
	...
	useEffect(() => {
		...
		if (active.kind === 'call') {
			...
			const isLikelyLocalRestore = hydratedAt ? Date.now() - hydratedAt < 1500 : false;
			if (isLikelyLocalRestore) {
				showToast('Call session is still active. Open Call to resume.');
			} else {
				void navigate('/call');
			}
		}
