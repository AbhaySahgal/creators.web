	...
	const requestSession = useCallback(
		(opts: {
			...
			const meta: OutgoingRequestMeta = {
				...
				...(opts.kind === 'call' && fanModality ? { uiCallType: fanModality } : opts.uiCallType ? { uiCallType: opts.uiCallType } : {}),
		...
		if (pendingOutgoing) {
			...
			const prevMeta =
				prevOutgoing.state === 'pending' || prevOutgoing.state === 'requesting' ?
					...
					{};
			const callModality = normalizeCallModality(pendingOutgoing.call_modality, pendingOutgoing.kind);
			if (callModality) callModalityByRequestIdRef.current[pendingOutgoing.id] = callModality;
			const uiCallType =
				prevMeta.uiCallType ??
				callModalityByRequestIdRef.current[pendingOutgoing.id] ??
				callModality;
			nextOutgoing = {
				state: 'pending',
				request: {
